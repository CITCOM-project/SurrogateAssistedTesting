...
from util.road_factory import get_road
from util.data_handler import get_values
from os import path
import copy
...
def move_config_file():
    cmd = ['docker', 'cp', cfg.base_directory+'e2e.conf', 'pylot:/home/erdos/workspace/pylot/configs/']
    run_command(cmd)
...
def run_single_scenario(fv_arg):
    ...
    while True:
        ...
        if counter > cfg.time_allowed or scenario_finished():
            ...
            stop_pylot_container()
            copy_to_host(fv)
            file_name = cfg.base_directory + 'Results/' + str(fv)
