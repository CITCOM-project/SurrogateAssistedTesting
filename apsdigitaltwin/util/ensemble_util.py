...
class Kriging:
    ...
    def create_model_from_cluster(self,cluster):
        ...
        X = cluster[:, 0:3] # features from 0 to 15th index
        y = cluster[:, 3:5] # value at 3th index
    ...
    def test(self, cluster):
        ...
        for i in range(len(cluster)):
            y_act = cluster[i][3]
            Y_pred = self.predict(cluster[i][:3])
...
class Polynomial_Regression:
    ...
    def create_model_from_cluster(self,cluster,deg):
        ...
        X = cluster[:, 0:3] # features from 0 to 15th index
        y = cluster[:, 3:5] # value at 16th index
        # y[y < 0] = 0
        # y[y > 1] = 1
...
class Ensemble:
    def __init__(self,database,deg=2):
        ...
        database = database[["start_bg", "start_cob", "start_iob", "hyper", "hypo"]].values.tolist()

        self.vars = ["start_bg", "start_cob", "start_iob"]
    ...
    def predict (self,fv):
        ...
        # return (y_rbf*self.w_rbf) + (y_pr*self.w_PR) + (y_kr*self.w_KR)
